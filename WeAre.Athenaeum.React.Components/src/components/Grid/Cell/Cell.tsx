...
import DateInput from "../../Form/Inputs/DateInput/DateInput";
import Dropdown, { DropdownAlign, DropdownOrderBy, DropdownVerticalAlign } from "../../Form/Inputs/Dropdown/Dropdown";
import NumberInput from "../../Form/Inputs/NumberInput/NumberInput";
import DropdownCell from "./DropdownCell/DropdownCell";
import { SelectListItem } from "../../Form/Inputs/Dropdown/SelectListItem";
import AddressInput from "../../Form/Inputs/AddressInput/AddressInput";
...
export default class Cell<TItem = {}> extends BaseComponent<ICellProps<TItem>> implements ICell {
                    ...
                    (hasInfoValue)
                        ...
                        (
                            <div className={gridStyles.twoLines}>
                                <span className={this.css(linkStyle, valueBoldStyle)} style={inlineStyles} onClick={async () => await this.invokeCallback(cell)} title={Localizer.get(title)}>
                                    {ReactUtility.toMultiLines(cellValue)}
                                </span>
                                <span style={inlineStyles} className={this.css(infoValueBold)} title={Localizer.get(infoTitle)}>
                        ...
                        :
                        (
                            <span className={this.css(linkStyle)} style={inlineStyles} onClick={async () => await this.invokeCallback(cell)} title={Localizer.get(title)}>
                                {ReactUtility.toMultiLines(cellValue)}
                            </span>
                        )
                }
    ...
    private renderAddressCellContent(cell: CellModel<TItem>, cellValue: string | null): React.ReactNode {
        return (
            <div className={this.css(gridStyles.textContainer)}>
            ...
            <div className={this.css(gridStyles.numberContainer, numberInfoStyle)}>
                {
                    (hasInfo) &&
                    (
                        <span className={this.css(hideZeroStyle)} title={Localizer.get(settings.infoTitle)}>
                            {Utility.formatValue(infoValue || 0, cell.column.format)}
                        </span>
                    )
                }
                <NumberInput clickToEdit
                             title={Localizer.get(cell.title)}
                    ...
                    (hasInfo && infoValue)
                        ...
                        (
                            <div className={gridStyles.twoLines}>
                                ...
                                />
                                <span title={Localizer.get(settings.infoTitle)}>
                                    {infoValue}
                                </span>
                            </div>
                        )
            ...
            if ((!readonly) && (!deleted)) {
                switch (column.type) {
                    ...
                    case ColumnType.Address:
                        cellStyle = gridStyles.textCell;
                        cellContent = this.renderAddressCellContent(cell, cellValue);
                        rendered = true;
                        editable = true;
                        break;
                }
