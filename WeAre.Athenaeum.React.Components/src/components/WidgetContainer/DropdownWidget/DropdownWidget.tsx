...
import Dropdown, {DropdownOrderBy, DropdownType, IDropdown} from "../../Form/Inputs/Dropdown/Dropdown";
        ...
        return (!this.props.noDescription)
            ? (this.selectedListItem != null)
                ? ((this.props.noSubtext) && (this.selectedListItem.subtext))
                    ? `${Localizer.get(this.selectedListItem.text)}, ${Localizer.get(this.selectedListItem.subtext)}`
                    : this.selectedListItem.text
                ...
                }
                {
                    ...
                    (
                        <Dropdown id={`dropdown_${this.id}`}
                                  ...
                                  onChange={async (sender, item, userInteraction) => await this.onChangeHandlerAsync(sender, item, userInteraction)}
                                  onChangeAmount={async (sender, item, amount) => await this.onChangeAmountHandlerAsync(sender, item, amount)}
                                  onFavoriteChange={async (sender, item, favorite) => await this.onFavoriteChangeAsync(sender, item, favorite)}
                                  onToggle={async (sender, expanded) => await this.onToggleHandlerAsync(sender, expanded)}
                        />
                    )
                }
            </div>
