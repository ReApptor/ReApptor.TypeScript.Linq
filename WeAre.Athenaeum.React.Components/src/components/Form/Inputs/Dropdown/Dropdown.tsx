...
import {IGlobalClick, IGlobalKeydown, ReactUtility, StylesUtility, TextAlign} from "@weare/athenaeum-react-common";
import BaseInput, {IBaseInputProps, IBaseInputState, ValidatorCallback} from "../BaseInput";
import Icon, {IconSize, IconStyle, IIconProps} from "@/components/Icon/Icon";
...
export default class Dropdown<TItem> extends BaseInput<DropdownValue, IDropdownProps<TItem>, IDropdownState> implements IGlobalClick, IGlobalKeydown, IDropdown {
    ...
    private async invokeOnChangeAsync(item: SelectListItem, userInteraction: boolean): Promise<void> {
        if (this.props.onChange) {
            ...
            const selectedItem: TItem = (item.selected)
                ? (item.ref != null)
                    ? item.ref
                    : item
                : null;
    ...
    private async onListContainerClick(e: React.MouseEvent): Promise<void> {
        e.stopPropagation();
    }
    ...
    public scrollToSelected(smooth: boolean = false): void {
        if (this.expanded) {
            const selectedItem: SelectListItem | undefined = this.selectedListItems.find(item => item.selected);
            if (selectedItem) {
                const scrollableContainerElement: Element | null = this._scrollableContainerRef.current;
    ...
    public renderInput(): React.ReactNode {
        ...
        return (
            <div className={this.css(dropdownStyle, dropdownWidgetStyle, transparentStyle, smallStyle, this.disabled && styles.disabled, "dropdown-container")} style={inlineStyles}>
                ...
                <div className={toggleStyle} onClick={(e: React.MouseEvent) => this.onListContainerClick(e)}>
