...
using Renta.Components.Common.Configuration.Settings;
        ...
        public TokenInfo Validate<TTokenData>(string jwtToken) where TTokenData : TokenData
        {
            ...
            if (validator.CanReadToken(jwtToken))
            {
                try
                {
                    ...
                    string username = principal.Claims.FirstOrDefault(c => c.Type == RentaConstants.ClaimTypes.NameIdentifier)?.Value;
                    string securityStampClaim = principal.Claims.FirstOrDefault(c => c.Type == RentaConstants.ClaimTypes.SecurityStamp)?.Value;
                    string tokenIdClaim = principal.Claims.FirstOrDefault(c => c.Type == RentaConstants.ClaimTypes.TokenId)?.Value;
                        ...
                        (!string.IsNullOrWhiteSpace(tokenIdClaim)) && (Guid.TryParse(tokenIdClaim, out Guid tokenId)) && (tokenId != Guid.Empty))
                    {
                        ...
                        if (principal.HasClaim(c => c.Type == RentaConstants.ClaimTypes.UserData))
                        {
                            string userDataJson = principal.Claims.First(c => c.Type == RentaConstants.ClaimTypes.UserData).Value;
